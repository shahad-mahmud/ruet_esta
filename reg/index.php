...
if( !(isset($_SESSION['uname']) && $_SESSION['logged'] == "TruE" && $_SESSION['post'] == "reg" )) //logged in for docket??
{
	...
	$sql		= 	"
					...
					FROM
					    ...
					    docket
					WHERE
					    move_history.docket_id = docket.docket_id AND docket.file_holder_no = file_holder.file_holder_no AND move_history.sent_from = destinations.destination_id AND move_history.destination_id = 15
					ORDER BY
					    move_history.date
					DESC
					    ,
		...
		if($_GET['action'] == "send")
		{
			if(isset($_GET['ref']))
			{
				...
				if(isset($_GET['id']))
				{
					...
					$sql2		= "INSERT INTO `move_history`(`docket_id`, `sent_from`, `destination_id`, `if_received`, `date`, `time`) VALUES ('$id', 15, '$ref', 0, '$date','$time' )";
